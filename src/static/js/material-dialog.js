	...
	*/
	dialog:
		"<div class='modal material-dialog class_here' style='width: 35%'>" +
			...
			"<div class='modal-footer article-modal'>" +
...
MaterialDialog.alert = function(text, options) {
	...
	let button_close = '<button class="btn modal-close button_class close">button_text</button>';
	let callback = null;
	const other_options = options;
	...
	if (callback) {
		const close = $('.material-alert .close').last();
		$(close).click(() => {
			callback.call();
		});
	}
...
MaterialDialog.dialog = function(text, options) {
	...
	const other_options = options;
	let body = this.templates.dialog;
